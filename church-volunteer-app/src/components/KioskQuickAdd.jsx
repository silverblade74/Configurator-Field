    ...
    <form onSubmit={handleSubmit} className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {batchMode && batchCount > 0 && (
          <span className="badge bg-blue-200 text-blue-800">{batchCount} added</span>
        )}
      </div>
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ref={nameRef}
          type="text"
          className="input flex-1 !min-h-[48px] text-base"
          value={name}
        ...
        <input
          type="tel"
          className="input w-full sm:w-36 !min-h-[48px]"
          value={phone}
          onChange={(e) => setPhone(e.target.value)}
          placeholder="Phone"
        />
        <select
          className="input w-full sm:w-40 !min-h-[48px]"
          value={department}
        ...
        <button
          ...
          className="btn-kiosk whitespace-nowrap !min-h-[48px]"
        >
