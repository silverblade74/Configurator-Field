...
export default function VolunteerDashboard() {
    ...
    async function loadData() {
      ...
      try {
        ...
        const upcoming = eventsWithDetails
          .filter((s) => s.event && s.event.date?.toDate() > now && s.status === 'signed_up')
          .sort((a, b) => a.event.date.toDate() - b.event.date.toDate())
          .slice(0, 5)

        const recent = eventsWithDetails
          .filter((s) => s.status === 'checked_out')
          .sort((a, b) => (b.checkedOutAt?.toDate() || 0) - (a.checkedOutAt?.toDate() || 0))
          .slice(0, 5)
  ...
  const earnedBadges = BADGE_DEFINITIONS.filter((b) =>
    (userProfile?.badges || []).includes(b.id)
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold">
            Welcome, {userProfile?.displayName || 'Volunteer'}!
          </h1>
          <p className="text-gray-500 mt-1">
            <span className={`font-medium ${level.color}`}>{level.name}</span> Level
            {nextMilestone && (
              <span className="text-gray-400"> &middot; Next milestone: {nextMilestone}h</span>
            )}
          </p>
        </div>
        <Link to="/events" className="btn-primary mt-3 sm:mt-0 inline-flex items-center space-x-1">
          <Calendar size={16} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Total Hours"
          value={formatHours(userProfile?.totalHours || 0)}
          icon={Clock}
          color="primary"
        />
        <StatCard
        ...
        <StatCard
          ...
          value={`${userProfile?.streak || 0} weeks`}
          icon={Flame}
          color="red"
        />
        <StatCard
          ...
          subtitle={`of ${BADGE_DEFINITIONS.length}`}
          icon={Trophy}
          color="purple"
        />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/events" className="text-primary-600 text-sm hover:underline flex items-center">
              View all <ArrowRight size={14} className="ml-1" />
            </Link>
          </div>
          {upcomingEvents.length === 0 ? (
            <p className="text-gray-400 text-sm py-4 text-center">
              No upcoming events. <Link to="/events" className="text-primary-600 hover:underline">Sign up for one!</Link>
            </p>
          ) : (
                ...
                <div key={signup.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {signup.event?.date?.toDate().toLocaleDateString('en-US', {
              ...
              {recentActivity.map((signup) => (
                <div key={signup.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="font-medium text-sm">{signup.event?.title}</p>
                    <p className="text-xs text-gray-500">
                      {signup.checkedOutAt?.toDate().toLocaleDateString('en-US', {
      ...
      {earnedBadges.length > 0 && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/badges" className="text-primary-600 text-sm hover:underline flex items-center">
              View all <ArrowRight size={14} className="ml-1" />
            </Link>
          </div>
