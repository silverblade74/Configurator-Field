...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(getAuthErrorMessage(err, 'Failed to log in. Please try again.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (!result?.redirecting) navigate(redirectTo)
    } catch (err) {
  ...
  async function handlePasswordReset() {
    ...
    try {
      ...
      setError(getAuthErrorMessage(err, 'Could not send reset email. Check the address and try again.'))
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <span className="text-5xl font-bold text-primary-700">VH</span>
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-right">
              <button type="button" className="text-sm text-primary-600 hover:underline" onClick={handlePasswordReset}>
                Forgot password?
              </button>
            </div>
          ...
          <button onClick={handleGoogle} disabled={loading} className="w-full flex items-center justify-center space-x-2 border border-gray-300 rounded-lg px-4 py-2 hover:bg-gray-50 transition-colors disabled:opacity-60">
