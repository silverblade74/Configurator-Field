...
export default function Leaderboard() {
  ...
  useEffect(() => {
    async function load() {
      try { const data = await getLeaderboard(50); setLeaders(data) }
  ...
  const sorted = [...leaders].sort((a, b) => sortBy === 'points' ? (b.totalPoints || 0) - (a.totalPoints || 0) : (b.totalHours || 0) - (a.totalHours || 0))
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={() => setSortBy('points')} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${sortBy === 'points' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-100'}`}>By Points</button>
          <button onClick={() => setSortBy('hours')} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${sortBy === 'hours' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-100'}`}>By Hours</button>
        </div>
      </div>

      {sorted.length === 0 ? (
        <EmptyState icon={Trophy} title="No volunteers yet" description="The leaderboard will populate as volunteers serve." />
      ) : (
        <>
          {sorted.length >= 3 && (
            <div className="hidden sm:grid grid-cols-3 gap-4">
              {[sorted[1], sorted[0], sorted[2]].map((user, idx) => {
                ...
                return (
                  <div key={user?.id || idx} className={`card text-center ${rank === 1 ? 'ring-2 ring-yellow-400 bg-yellow-50' : ''} ${isMe ? 'ring-2 ring-primary-400' : ''}`}>
                    <div className="flex justify-center mb-2">{getRankIcon(rank)}</div>
                    <div className="w-12 h-12 mx-auto rounded-full bg-gray-200 flex items-center justify-center text-lg font-bold text-gray-600">{user?.displayName?.charAt(0)?.toUpperCase() || '?'}</div>
                    ...
                    <div className="mt-2 text-sm">
                      ...
                      <p className="text-xs text-gray-400">{sortBy === 'points' ? formatHours(user?.totalHours || 0) : `${user?.totalPoints || 0} pts`}</p>
                    </div>
                  </div>
                )
              })}
                ...
                {sorted.map((user, idx) => {
                  ...
                  return (
                    <tr key={user.id} className={isMe ? 'bg-primary-50' : 'hover:bg-gray-50'}>
                      <td className="px-4 py-3 w-12"><div className="flex items-center">{getRankIcon(idx + 1)}</div></td>
                      <td className="px-4 py-3">
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-sm font-bold text-gray-600">{user.displayName?.charAt(0)?.toUpperCase() || '?'}</div>
                          <div><p className="font-medium text-sm">{user.displayName}{isMe && <span className="text-primary-600 ml-1">(You)</span>}</p></div>
                        </div>
                      </td>
