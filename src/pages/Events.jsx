...
export default function Events() {
  ...
  async function loadData() {
    try {
      const [eventsData, ministriesData, signupsData] = await Promise.all([
        getEvents(), getMinistries(), userProfile ? getUserSignups(userProfile.uid || userProfile.id) : [],
      ])
      setEvents(eventsData); setMinistries(ministriesData); setUserSignups(signupsData)
  ...
  async function handleSignUp(eventId) {
    ...
    try { await signUpForEvent(eventId, userProfile.uid || userProfile.id, userProfile.displayName); await loadData(); toast.success('Signed up!') }
  ...
  async function handleCancel(eventId) {
    ...
    if (!signup) return
    setActionLoading(eventId)
    try { await cancelSignup(signup.id, eventId); await loadData(); toast.info('Signup cancelled') }
    catch (err) { toast.error('Failed to cancel signup') }
    setActionLoading(null)
  }
  ...
  function isSignedUp(eventId) { return userSignups.some((s) => s.eventId === eventId && s.status !== 'checked_out') }
  function getMinistryName(ministryId) { return ministries.find((m) => m.id === ministryId)?.name || 'General' }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <select className="input w-auto" value={filter} onChange={(e) => setFilter(e.target.value)}>
            <option value="upcoming">Upcoming</option><option value="past">Past</option><option value="all">All Events</option>
          </select>
            ...
            return (
              <div key={event.id} className={`card ${isPast ? 'opacity-75' : ''}`}>
                ...
                {!isPast && (
                  <div className="mt-4">
                    {signed ? (
                      <button onClick={() => handleCancel(event.id)} disabled={actionLoading === event.id} className="btn-secondary w-full flex items-center justify-center space-x-1"><X size={14} /><span>{actionLoading === event.id ? 'Cancelling...' : 'Cancel Signup'}</span></button>
                    ) : (
                      <button onClick={() => handleSignUp(event.id)} disabled={actionLoading === event.id || (event.maxVolunteers && event.signupCount >= event.maxVolunteers)} className="btn-primary w-full">{actionLoading === event.id ? 'Signing up...' : 'Sign Up'}</button>
                    )}
                  </div>
                )}
              </div>
