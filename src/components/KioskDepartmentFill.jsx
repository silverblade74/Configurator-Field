  ...
  return (
    <div className="flex flex-wrap gap-2">
      {activeDepts.map((dept) => (
        <div key={dept.id} className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-lg px-3 py-1.5">
          <span className="text-sm">{dept.icon}</span>
          <span className="text-sm font-medium text-gray-700">{dept.name}</span>
          <span className="text-sm font-bold text-primary-600">{deptCounts[dept.id] || 0}</span>
        </div>
      ))}
    </div>
  )
